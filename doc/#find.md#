...
The *find* object extends [instruction](instruction.md), and permits all of its attributes.  Take a look at the [schema](../schema/json/find) for technical specifics.
...
This would search for all matches of "template regexp" within the text provided by its enclosing expression.  If *find* also has a *name*, each match will be saved to the database and used for substitutions.  If it has *then* children, each child will be run once with every match.
