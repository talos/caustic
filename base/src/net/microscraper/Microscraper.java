...
public class Microscraper extends Log {
	 ...
	 * @param jsonInterface A {@link JSONInterface} to use in parsing and loading JSON.
	...
	public Microscraper(RegexpCompiler compiler, Browser browser, URIFactory uriFactory,
			JSONInterface jsonInterface, Database database) {
	...
	private void scrape(JSONInterfaceObject pageJson, String urlEncodedDefaults, Hashtable extraDefaults) throws MicroscraperException {
		try {
			...
			Executable exc = new Executable(page, compiler,
	...
	public void scrapeWithJSON(String pageInstructionJSON) throws MicroscraperException {
		JSONInterfaceObject json = jsonInterface.parse(uriFactory.fromString(userDir), pageInstructionJSON);
	...
	public void scrapeWithURI(String uri) throws MicroscraperException {
		JSONInterfaceObject json = jsonInterface.load(this.uriFactory.fromString(uri));
		scrape(json, null, null);
	}
	/**
	 * Scrape from a {@link Page} in a JSON String.
	...
	/**
	 * {@link #execute} runs an {@link Executable} and its children.
